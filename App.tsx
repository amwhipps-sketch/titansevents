...
const App: React.FC = () => {
  ...
  const { upcomingEvents, nextDayEvents } = useMemo(() => {
      ...
      const filtered = fixtures.filter(f => {
          ...
          if (!isUpcoming || isTraining) return false;
      ...
      if (filtered.length > 0) {
          ...
          // Show ALL visible events on the next scheduled date
          featuredEvents = filtered.filter(f => 
            f.date.getFullYear() === nextDate.getFullYear() &&
            f.date.getMonth() === nextDate.getMonth() &&
            f.date.getDate() === nextDate.getDate()
          );
      }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 pb-12" id="app">
        <Header 
